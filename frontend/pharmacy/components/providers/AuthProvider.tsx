  ...
  if (!publishableKey) {
    return (
      <main className={styles.authConfigurationError}>
        ...
        <p>
          Add <code>NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY</code> to the frontend environment before opening the pharmacy
          workspace.
        </p>
      </main>
    );
  ...
  if (state !== "authorized") {
    return (
      <main className={styles.staffAccessState} role={state === "checking" ? "status" : "alert"}>
        ...
        <h1>{state === "checking" ? "Checking clinic access" : "Pharmacist access required"}</h1>
        <p>{message}</p>
        <UserButton showName />
      </main>
    );
